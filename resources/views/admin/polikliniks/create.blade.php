...
<div class="mb-8">
    <div class="flex items-center gap-4 mb-2">
        ...
        <h2 class="text-3xl font-bold text-slate-900 tracking-tight">Tambah Poliklinik</h2>
    </div>
    <p class="text-slate-500 font-medium ml-14">Masukkan informasi poliklinik baru.</p>
</div>
    ...
    <form action="{{ route('admin.polikliniks.store') }}" method="POST">
        ...
        <div class="p-8 space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Klinik Penyakit Dalam" required
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" placeholder="Jelaskan jenis perawatan atau fasilitas di poliklinik ini..." 
                          class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 font-medium transition-all text-slate-800 resize-y">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-rose-500 text-xs mt-2 font-bold">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="px-8 py-5 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-900/20">Simpan Data</button>
        </div>
    </form>
</div>
@endsection
